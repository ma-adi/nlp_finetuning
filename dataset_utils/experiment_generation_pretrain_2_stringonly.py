...
EXPERIMENTS = {
    ...
    'easy_master_curriculum_2_nested': {
        'description': "More realistic compositional testing for master_curriculum -- same core formats",
        'core_formats': [
            'SingleField', 'AttributedEntity', 'BasicEntity', 'PropertyList',
            'ListOfSimpleEntities', 
            ...
            'SpecialValues', 'NamespacedObject', 'HeterogeneousList', 'CDataField',
            'ProcessingInstructionNode', 'EntityReferenceField','HintedEmptyNode',
            'AttributePromotedKeyValueList', 'ListOfAttributedEntities', 'DeeplyNested'
        ],
        'ap_formats': ['NestedAttributedEntity', 'ListWithSpecialValues', 
                    'AttributedPropertyList', 'ListOfObjectsWithSpecialValues',
                    'NamespacedAttributes', 'ListOfMixedContentObjects']
...
EXPERIMENT_TO_RUN = 'easy_master_curriculum_2'
...
### REVISED - SpecialValues (String-Only Policy) ###
class SpecialValues(FormatTemplate):
    """
    Teaches the boundary between text content and nulls under a strict
    "everything is a string" policy.
    1. An empty element (<tag/> or <tag></tag>) becomes null.
    2. Any element with text, including "true" or "false", becomes a string.
    """
    def __init__(self, **kwargs):
        super().__init__('SpecialValues', ['simple_kv', 'empty_element_to_null'], **kwargs)
    ...
    def generate_example(self, difficulty: str = 'medium'):
        ...
        # Rule 1: Text "true" is now a STRING "true"
        admin_el = ET.SubElement(root, nm['is_admin'])
        admin_el.text = 'true'
        json_data[nm['is_admin']] = 'true' # <-- THE FIX: No longer a boolean

        # Rule 2: Text "false" is now a STRING "false"
        guest_el = ET.SubElement(root, nm['is_guest'])
        guest_el.text = 'false'
        json_data[nm['is_guest']] = 'false' # <-- THE FIX: No longer a boolean
        
        # Rule 3: Empty element -> null (This is the allowed exception)
...
if __name__ == '__main__':
    ...
    filename = f'dataset_utils/tagged_dataset_exp_{EXPERIMENT_TO_RUN}_{N_EXAMPLES}_{grammar_part}_{indent_part}_v2_emptylistdict_stringonly.json'
